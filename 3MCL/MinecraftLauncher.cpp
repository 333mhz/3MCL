...
void MinecraftLauncher::check_and_copy_files()
{
	...
	if (find_obj != INVALID_HANDLE_VALUE)
	{
		do
		{
			...
			if (find_data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
			{
				if (_wcsicmp(find_data.cFileName, L"assets") == 0
					|| _wcsicmp(find_data.cFileName, L"libraries") == 0
					|| _wcsicmp(find_data.cFileName, L"logs") == 0
					|| _wcsicmp(find_data.cFileName, L"crash-reports") == 0
					|| _wcsicmp(find_data.cFileName, L"versions") == 0)
				{
